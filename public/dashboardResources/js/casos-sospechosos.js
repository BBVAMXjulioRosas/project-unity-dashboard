...
(function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);throw new Error("Cannot find module '"+o+"'")}var f=n[o]={exports:{}};t[o][0].call(f.exports,function(e){var n=t[o][1][e];return s(n?n:e)},f,f.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){
	...
	var Sospechosos = require('./sospechosos/dashboard-casos.js');
	
	var sospechosos= new Sospechosos();
	},{"./sospechosos/dashboard-casos.js":6,"jquery":21}],2:[function(require,module,exports){
	...
	Button.prototype.setClaseActivo = function(clase){
		$(this.selector).addClass(clase)
	}
	
	Button.prototype.removeClaseActiva = function(clase){
		$(this.selector).removeClass(clase)
	}
	...
	Charts.prototype.renderBarChart = function (labels, sospechosos, sospechososNuevos) {
		...
		labels.forEach(element => {
			coloresSos.push(this.colores[0])
			coloresCon.push(this.colores[1])
		});
		...
		this.graficaBar = new Chart(ctx, {
			...
			data: {
				...
				datasets: [
					{
						label: "Total Nuevos Sospechosos",
						data: sospechosos,
						backgroundColor: coloresSos
					},
					{
						label: "Nuevos Sospechosos Tras Filtro",
						data: sospechososNuevos,
						backgroundColor: coloresCon
					}
				],
			},
			options: {
	...
	Charts.prototype.renderStackedChart = function (labels, sospechosos, sospechososNuevos) {
		let ctx = this.element;
		...
		labels.forEach(element => {
			coloresSos.push(this.colores[1])
			coloresCon.push(this.colores[0])
		});
	...
	},{"chart.js":20}],6:[function(require,module,exports){
	var Servicios = require('../registro/servicios');
	var SelectComponent = require('../registro/selectComponent');
	var Button = require('./button');
	var Charts = require('./chartSospechosos');
	var boostrap = require('bootstrap');
	
	var Sospechosos = function () {
		//this.model = Model;
		...
		this.valorInicial = []
		this.valorInicialEdad = []
		this.valorInicialStacked = []
		this.valorInicialEdadStacked = []
		...
		this.genero = new SelectComponent("#genero");
		this.buttonAll = new Button('#total-button');
		this.monthButton = new Button('#month-button');
		this.weekButton = new Button('#week-button');
		this.bar1 = new Charts("#bar1");
		this.stackedBar1 = new Charts("#stackedBar1");
		this.bar2 = new Charts("#bar2");
		this.stackedBar2 = new Charts("#stackedBar2");
		...
		this.genero.element.addEventListener("change", this.eventosGenero.bind(this));
		this.buttonAll.element.addEventListener("click", this.setTemporalidad.bind(this));
		this.monthButton.element.addEventListener("click", this.setTemporalidad.bind(this));
		this.weekButton.element.addEventListener("click", this.setTemporalidad.bind(this));
		...
		document.getElementById("diabetes").addEventListener("change", this.setFiltroFactorRiesgo.bind(this))
		document.getElementById("hiperten").addEventListener("change", this.setFiltroFactorRiesgo.bind(this))
		document.getElementById("obesidad").addEventListener("change", this.setFiltroFactorRiesgo.bind(this))
		document.getElementById("defensas").addEventListener("change", this.setFiltroFactorRiesgo.bind(this))
		document.getElementById("embarazada").addEventListener("change", this.setFiltroFactorRiesgo.bind(this))
	
	
		document.getElementById('reiniciarFiltros').addEventListener('click', this.refreshFilters.bind(this));
	
		//Se manda a pintar las gráficas luego de responder los servicios
		//this.chartBar1();
		//this.chartBar2();
	
		this.servicios.getCasosSospechososFecha(this.filtros).then((data) => {
			this.valorInicial = data
			this.renderBarChart1(data)
			this.renderStackedBar1(data)
		})
	
		this.servicios.getCasosSospechososEdad(this.filtros).then((data) => {
			this.valorInicialEdad = data
			this.renderBarChart2(data)
			this.renderStackedBar2(data)
		})
	
		console.log("Valor inicial" + JSON.stringify(this.valorInicial))
	}
	
	//Funciones Generales 
	
	//Funcion que renicia los filtros que se hayan modificado en una interacción anterior
	Sospechosos.prototype.refreshFilters = function () {
		this.filtros.municipio = undefined,
			...
			this.filtros.genero = undefined,
			this.filtros.diabetes = 0,
			this.filtros.obesidad = 0,
			this.filtros.hipertension = 0,
			this.filtros.defensas = 0,
			this.filtros.embarazada = 0
	
		this.setFechaActual();
	
		this.estados.cleanFilterEstados();
		this.chartBar1();
		this.chartBar2();
	
		this.genero.selectTexto("all");
	
		this.buttonAll.setClaseActivo("boton-period-selected");
		this.monthButton.removeClaseActiva("boton-period-selected");
		this.weekButton.removeClaseActiva("boton-period-selected");
	
		document.getElementById("diabetes").checked = false
		document.getElementById("hiperten").checked = false
		document.getElementById("obesidad").checked = false
		document.getElementById("defensas").checked = false
		document.getElementById("embarazada").checked = false
	}
	
	Sospechosos.prototype.setFechaActual = function () {
		let date = new Date(); //Fecha actual
		this.fechaPasada(date); // Inserta Semana Pasada en el campo
	...
	Sospechosos.prototype.fechaPasada = function () {
		let initDate = new Date('2020-01-01');
		let fechaFormateada = this.formatoFechaInputs(initDate);
		document.getElementById('fechaIni').value = fechaFormateada;
		this.filtros.fechaIni = this.fechaAmilisegundos(fechaFormateada);
	}
	
	Sospechosos.prototype.fechaSemana = function(fechaActual) {
	...
	Sospechosos.prototype.fechaUnMes = function (fechaActual) {
		let date = new Date(); //Fecha actual
		let haceUnMes = new Date(fechaActual.getTime() - 2629750000);
	
		let fechaHaceUnMes = this.formatoFechaInputs(haceUnMes)
		let fechaHoy = this.formatoFechaInputs(date)
	
		document.getElementById('fechaIni').value = fechaHaceUnMes
		document.getElementById('fechaFin').value = fechaHoy
	
	
		this.filtros.fechaIni = this.fechaAmilisegundos(fechaHaceUnMes + ' 23:59:59');
		this.filtros.fechaFin = this.fechaAmilisegundos(fechaHoy + ' 23:59:59');
	}
	
	//Fecha a Milisegundos
	Sospechosos.prototype.fechaAmilisegundos = function (date) {
	...
	Sospechosos.prototype.formatearFecha = function (date) {
		...
		return date = day + "-" + meses[month];
	}
	
	//Formatea las fechas a formato (YYYY-mm-DD)
	Sospechosos.prototype.formatoFechaInputs = function (date) {
	...
	Sospechosos.prototype.eventosEstados = function () {
		this.filtros.municipio = undefined;
		this.filtros.colonia = undefined;
	...
	Sospechosos.prototype.eventosMunicipios = function () {
		this.filtros.colonia = undefined;
	...
	Sospechosos.prototype.eventosColonias = function () {
	...
	Sospechosos.prototype.eventosGenero = function (value) {
		if (this.genero.element.value !== "all") {
			this.filtros.genero = this.genero.element.value;
		} else {
			this.filtros.genero = undefined
		}
	...
	Sospechosos.prototype.setTemporalidad = function (event) {
		...
		switch (idButton) {
			case "total-button":
				//Limpia filtros de fecha y ejecuta servicios sin rango de fechas
				this.filtros.fechaIni = undefined;
				this.filtros.fechaFin = undefined;
				this.setFechaActual();
				...
				break
			case "month-button":
				this.monthButton.setClaseActivo("boton-period-selected")
				this.buttonAll.removeClaseActiva("boton-period-selected")
				this.weekButton.removeClaseActiva("boton-period-selected")
	
				this.fechaUnMes(date)
	
	
				//Setea Fechas en un rango de un mes
				break
			default:
				this.weekButton.setClaseActivo("boton-period-selected")
				this.buttonAll.removeClaseActiva("boton-period-selected")
				this.monthButton.removeClaseActiva("boton-period-selected")
	
				this.fechaSemana(date)
	
				//Setea Fechas en un rango de una semana
				break
		}
	...
	Sospechosos.prototype.filtroPorFecha = function (value){
	...
	Sospechosos.prototype.setFiltroFactorRiesgo = function (event) {
		...
		switch (idCheckbox) {
			...
			case "diabetes":
				this.filtros.diabetes = event.target.checked ? 1 : 0
	
				break
			case "hiperten":
				this.filtros.hipertension = event.target.checked ? 1 : 0
	
				break
			case "obesidad":
				this.filtros.obesidad = event.target.checked ? 1 : 0
	
				break
			case "defensas":
				this.filtros.defensas = event.target.checked ? 1 : 0
	
				break
	
			default:
				this.filtros.embarazada = event.target.checked ? 1 : 0
	...
	//Llamadas a funciones AJAX
	
	//Carga el catálogo de Delegaciones/Municipios
	Sospechosos.prototype.getEstados = function () {
		this.servicios.getStates().then((data) => {
			this.fillEstados(data);
		});
	}
	
	Sospechosos.prototype.getMunicipios = function () {
	...
	Sospechosos.prototype.getColonias = function () {
	...
	Sospechosos.prototype.fillEstados = function (collection) {
		this.estados.fill(collection);
	}
	
	//Llena el Select de Delegaciones Municipios
	Sospechosos.prototype.fillMunicipios = function (collection) {
		this.municipios.fill(collection)
	}
	
	//Llena el Select de Colonias Asentamientos
	Sospechosos.prototype.fillColonias = function (collection) {
		this.colonias.fill(collection)
	}
	
	Sospechosos.prototype.fillGenero = function (collection) {
		this.genero.fillGeneral(this.generoFilter);
	}
	
	//Funciones y mapeos para Graficas
	
	Sospechosos.prototype.chartBar1 = function () {
		this.servicios.getCasosSospechososFecha(this.filtros).then((data) => {
			this.renderBarChart1(data)
			this.renderStackedBar1(data)
		})
	...
	Sospechosos.prototype.chartBar2 = function () {
		this.servicios.getCasosSospechososEdad(this.filtros).then((data)=>{
			this.renderBarChart2(data)
			this.renderStackedBar2(data)
		})
	...
	Sospechosos.prototype.renderBarChart1 = function (data) {
		//Primer Gráfica
		let sospechosos = [];
		let sospechososNuevos = [];
		let etiquetas = [];
	
		this.valorInicial.forEach((element,idx) => {
			sospechosos.push(element.totalSuspect.value)
			etiquetas.push(this.formatearFecha(element.date))
		})
	
		this.valorInicial.forEach(element =>{
			let elemento2 = data.filter(el => el.date === element.date)
			
			if(elemento2.length !== 0){
				sospechososNuevos.push(elemento2[0].totalSuspect.value)
			}else{
				sospechososNuevos.push(0)
			}
		})
	
	
		this.bar1.renderBarChart(etiquetas, sospechosos, sospechososNuevos)
	}
	
	Sospechosos.prototype.renderStackedBar1 = function (data) {
		//Segunda Gráfica
		let sospechosos = [];
		let sospechososNuevos = [];
		let etiquetas = [];
		...
		if(Object.keys(this.valorInicialStacked).length !== 0 ){
			this.valorInicialStacked.forEach(element =>{
				let elemento2 = data.filter(el => el.date === element.date)
				
				if(elemento2.length !== 0){
					if(elemento2[0].totalSuspect.percentage !== 100){
						sospechososNuevos.push(elemento2[0].totalSuspect.percentage)
					}else{
						sospechososNuevos.push(0)
					}
				}else{
			...
			this.valorInicialStacked = data
		}
	
		this.valorInicialStacked.forEach(element => {
			sospechosos.push(element.totalSuspect.percentage)
			etiquetas.push(this.formatearFecha(element.date))
		})
	
		sospechosos = this.valorInicialStacked.map((item, index) =>{
			return sospechososNuevos[index] !== undefined ? item.totalSuspect.percentage - sospechososNuevos[index] : item.totalSuspect.percentage
		})
	
		//Logica para mapear respuesta de servicio y pintar primer gráfico
		this.stackedBar1.renderStackedChart(etiquetas, sospechosos, sospechososNuevos)
	}
	
	Sospechosos.prototype.renderBarChart2 = function (data) {
		...
		data.forEach(element => {
			sospechososNuevos.push(element.totalSuspect.value)
		});
	
		this.valorInicialEdad.forEach(element => {
			sospechosos.push(element.totalSuspect.value)
			etiquetas.push(element.age)
	
		})
	
		//Logica para mapear respuesta de servicio y pintar primer gráfico
		this.bar2.renderBarChart(etiquetas, sospechosos, sospechososNuevos)
	}
	
	Sospechosos.prototype.renderStackedBar2 = function (data) {
		//Segunda Gráfica
		let sospechosos = [];
		let sospechososNuevos = [];
		let etiquetas = [];
	
		if(Object.keys(this.valorInicialEdadStacked).length !== 0 ){
			data.forEach(element => {
				if (element.totalSuspect.percentage !== 100){
					sospechososNuevos.push(element.totalSuspect.percentage)
				}
			});
		}else {
			this.valorInicialEdadStacked = data
		}
	
		this.valorInicialEdadStacked.forEach(element => {
			sospechosos.push(element.totalSuspect.percentage)
			etiquetas.push(element.age)
		})
	
		sospechosos = this.valorInicialEdadStacked.map((item, index) =>{
			return sospechososNuevos[index] !== undefined ? item.totalSuspect.percentage - sospechososNuevos[index] : item.totalSuspect.percentage
		})
	
		//Logica para mapear respuesta de servicio y pintar primer gráfico
		this.stackedBar2.renderStackedChart(etiquetas, sospechosos, sospechososNuevos)
	}
	
	module.exports = Sospechosos;
	
	},{"../registro/selectComponent":2,"../registro/servicios":3,"./button":4,"./chartSospechosos":5,"bootstrap":7}],7:[function(require,module,exports){
	...
	*/
	...
	},{"moment":22}],21:[function(require,module,exports){
	/*!
